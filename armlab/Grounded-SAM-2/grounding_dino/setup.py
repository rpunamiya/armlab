...
def get_extensions():
    ...
    """
    CPU-only build of groundingdino._C:  
    compile all .cpp exactly once, skip any .cu files.
    """
    import glob, os
    ...
    # The “main” file
    main_source = os.path.join(extensions_dir, "vision.cpp")
    # Grab every .cpp under csrc (including subfolders), then exclude vision.cpp
    other_sources = [
        p for p in glob.glob(os.path.join(extensions_dir, "**", "*.cpp"), recursive=True)
        if os.path.abspath(p) != os.path.abspath(main_source)
    ]

    # Now build the final, de-duplicated list
    sources = [main_source] + other_sources

    include_dirs     = [extensions_dir]
    define_macros    = []       # no special macros
    extra_compile_args = {"cxx": []}

    print("Building groundingdino._C as CPU-only CppExtension")
    return [
        CppExtension(
